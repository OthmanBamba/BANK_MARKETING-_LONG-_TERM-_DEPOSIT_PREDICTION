    ...
    if expected_features:
        ...
        if missing_columns:
            ...
            if st.sidebar.button("Prédire"):
                ...
                st.write("### Résultats des prédictions sur les décisions d'offre de DAT (0 pour refus / 1 pour acceptation)")
                st.dataframe(df[expected_features + ["Prédiction"]])

                # Visualisation des prédictions
                st.write("### Distribution des prédictions sur les décisions d'offre de DAT")
                fig, ax = plt.subplots()
                sns.countplot(x=df["Prédiction"], ax=ax)
                st.pyplot(fig)

                # Visualisation des variables impactantes
                st.write("### Impact des variables sur les décisions d'offre de DAT")
                fig, ax = plt.subplots()
                ...
                # **Ajouter l'évolution du taux d'acceptation des DAT par nombre de personnes appelées**
                st.write("### Évolution du taux d'acceptation des DAT par nombre de personnes appelées")

                # Tri des données en fonction du nombre d'appels effectués, par exemple 'campaign' ou 'duration'
                df_sorted = df.sort_values(by="campaign")  # ou 'duration' selon votre préférence

                # Ajouter une colonne représentant le cumul des appels effectués
                df_sorted['Cumul_appels'] = np.arange(1, len(df_sorted) + 1)

                # Calculer le taux d'acceptation cumulatif
                df_sorted['Taux_acceptation_cumul'] = df_sorted['Prédiction'].cumsum() / df_sorted['Cumul_appels'] * 100

                # Tracer l'évolution du taux d'acceptation
                fig, ax = plt.subplots(figsize=(10, 6))
                ax.plot(df_sorted['Cumul_appels'], df_sorted['Taux_acceptation_cumul'], color='blue', label="Taux d'acceptation")
                ax.set_xlabel("Nombre de personnes appelées")
                ax.set_ylabel("Taux d'acceptation (%)")
                ax.set_title("Évolution du taux d'acceptation des DAT au fil des appels")
                ax.legend()
